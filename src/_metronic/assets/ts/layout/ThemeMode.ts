...
class ThemeMode {
  ...
  public getMode = (): Mode => {
    ...
    const ls = sessionStorage.getItem(modeParam)
    if (ls) {
      return ls as Mode
    }
  ...
  public setMode = (mode: Mode, menuMode: Mode | ''): void => {
    ...
    if (sessionStorage) {
      sessionStorage.setItem(modeParam, mode)
    }

    // Set active menu item
    if (activeMenuItem && sessionStorage) {
      sessionStorage.setItem(menuModeParam, menuMode)
  ...
  public getMenuMode = (): Mode | '' => {
    ...
    const ls = sessionStorage ? sessionStorage.getItem(menuModeParam) : null
    return (ls as Mode) || ''
  }
  ...
  private setActiveMenuItem = (item: HTMLElement): void => {
    ...
    if (sessionStorage && menuMode && menuModeParam) {
      sessionStorage.setItem(menuModeParam, menuMode)
    }
  }
